  ...
  useEffect(() => {
    const run = async () => {
      ...
      if (selectedTime === 'week') {
        startTime = moment().day(0 + 7 * selectWeek).hour(0).minute(0).second(0).format().split('+')[0];
        endTime = moment().day(7 + 7 * selectWeek).hour(0).minute(0).second(0).format().split('+')[0];
      }
      ...
      if (selectedTime === 'month') {
        ...
        endTime = moment().week(weeksInYear).day(7 * (weeksInMonth - 1)).hour(0).minute(0).second(0).format().split('+')[0]
      }

      //* year
      if (selectedTime === 'year') {
        startTime = moment().year(thisYear + selectWeek).month(0).date(1).hour(0).minute(0).second(0).format().split('+')[0]
        endTime = moment().year(thisYear + selectWeek).month(12).date(0).hour(0).minute(0).second(0).format().split('+')[0]
      }
      console.log(`startTime`, startTime)
      console.log(`endTime`, endTime)

      const { data: { order, percent, sortCategoryResult } } = await axios.get(`/transaction?startTime=${startTime}&endTime=${endTime}&selectedTime=${selectedTime}`)
      ...
      //* set showtime
      selectedTime === 'day' || setShowTime([startTime.split('T')[0].split('-').join('/'), endTime.split('T')[0].split('-').join('/')])
    ...
    <div className='transactionPage'>
      <div className="container" style={{ width: '80vw' }}> {/*revenue*/}
        <div className="my-5 p-4">
          ...
          <div className="row">
            <div className="col-8 card my-2"> {/*graph*/}
              <div className="row d-flx justify-content-end"> {/*selector*/}
                <select onChange={e => setSelectedTime(e.target.value)} style={{ width: '10vw', textAlign: 'center' }} className="form-select">
              ...
              <div className="row px-4">  {/*INCOME*/}
                <div>
                  <h5 style={{ fontSize: '24px' }}>INCOME</h5>
                  <div className="row">
                    <div className='d-flex align-items-center col-4'>
                      <h6 className='m-0' style={{ fontSize: '24px' }}>{`$${showSumIncome}`}</h6>
                      <i className={`ms-2 text-${percent >= 0 ? 'success' : 'danger'} fs-3 bi bi-caret-${percent >= 0 ? 'up' : 'down'}-fill`}></i>
                      <p className={`m-0 text-${percent >= 0 ? 'success' : 'danger'}`} style={{ fontSize: '24px' }}>{`${percent}%`}</p>
                    </div>
                    <div className='col-8 d-flex align-items-center justify-content-center flex-nowrap'>
                      <span><i onClick={() => setSelectWeek(cur => cur - 1)} className="fs-5 btn mx-2 bi bi-chevron-left"></i></span>
                      <span style={{ fontSize: '24px' }}>{selectedTime === 'day' ? `${showTime[0]}` : `${showTime[0]} - ${showTime[1]}`}</span> {/*showtime*/}
                      <span><i onClick={() => setSelectWeek(cur => cur + 1)} className="fs-5 btn mx-2 bi bi-chevron-right"></i></span>
                    </div>
                  </div>
                  <div className='p-3'>
      ...
      <div className="container" style={{ width: '80vw', marginBottom: '6vw' }}> {/*Previous Transaction*/}
