...
const RevenueChart = ({ order, selectedTime, selectWeek }) => {
  ...
  if (selectedTime === 'day') {
    ...
    const hours = ['0AM', '2AM', '4AM', '6AM', '8AM', '10AM', '12AM', '0PM', '2PM', '4PM', '6PM', '8PM', '10PM', '12PM']
    ...
    order.forEach((item) => {
      for (let i = 0; i < 12; i++) {
        if (new Date(item.paidAt).getTime() >= new Date(filterDay[i]).getTime() && new Date(item.paidAt).getTime() < new Date(filterDay[i + 1]).getTime()) {
  ...
  if (selectedTime === 'week') {
    ...
    labels = [...days];
    order.forEach(item => { //making revenue for week
  ...
  if (selectedTime === 'month') {
    ...
    for (let i = 0; i <= weeksInMonth; i++) {
      filterWeek.push(moment.utc().week(weeksInYear).day(0 + 7 * i).hour(0).minute(0).second(0).format())
    }
    //* making revenue for month
    order.forEach((item) => {
      for (let i = 0; i < weeksInMonth - 1; i++) {
        if (new Date(item.paidAt).getTime() >= new Date(filterWeek[i]).getTime() && new Date(item.paidAt).getTime() < new Date(filterWeek[i + 1]).getTime()) {
          Newrevenue[i] += +item.amount;
        }
      }
    })
